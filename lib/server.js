...
var Bitcore_ = Utils.getBitCoreAll();
...
WalletService.prototype._getBlockchainExplorer = function(coin, network) {
    ...
    if (this.blockchainExplorerOpts && this.blockchainExplorerOpts[coin] && this.blockchainExplorerOpts[coin][network]) {
        ...
        throw new Error("no config..." + coin + "," + network);
    ...
    async.series([
        ...
        function(next) {
            ...
            _.each(allUtxos, function(utxo) {
                if (addressToPath && addressToPath[utxo.address]) {
                    utxo.path = addressToPath[utxo.address].path;
                    utxo.publicKeys = addressToPath[utxo.address].publicKeys;
                } else {
                    log.error("addressToPath", addressToPath);
                    cb("error no Address To Path", allUtxos);
                }
...
WalletService.prototype.getSendMaxInfo = function(opts, cb) {
    ...
    self.getWallet({}, function(err, wallet) {
        ...
        if (!feeLevels)
            feeLevels = Defaults.FEE_LEVELS['btc'];
        if (opts.feeLevel) {
            if (!_.any(feeLevels, {
                    name: opts.feeLevel
                }))
...
WalletService._initBlockchainHeightCache = function() {
    ...
    var _cachedBlockheight = {};

    for (var index in _networks) {
        var net = _networks[index];
        if (net.coin) {
            _cachedBlockheight[net.coin] = {};
            _cachedBlockheight[net.coin][net.name] = {};
        }
    }
    WalletService._cachedBlockheight = _cachedBlockheight;
    // WalletService._cachedBlockheight = {
    //     btc: {
    //         livenet: {},
    //         testnet: {}
...
WalletService._clearBlockchainHeightCache = function(coin, network) {
    ...
    if (WalletService._cachedBlockheight[coin] && WalletService._cachedBlockheight[coin][network])
